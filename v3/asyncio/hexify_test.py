...
def test_hexify_data():
    ...
    assert next(h) == '80001234 00000000 2A AB CD                                        |*..|\n'

def test_hexify_multiline_data():
    h = hexify.Hexify(16).hexify_data(range(17), 0xC0DE)
    assert next(h) == '0000C0DE 00000000 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F |................|\n'
    assert next(h) == '0000C0EE 00000010 10                                              |.|\n'

def test_hexify():
    h = hexify.Hexify(16).hexify(b'\xAB\xCD', 0xBEAF)
    assert next(h) == header1
    assert next(h) == header2
    assert next(h) == '0000BEAF 00000000 AB CD                                           |..|\n'
